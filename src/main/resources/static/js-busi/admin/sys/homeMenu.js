...
$(function() {
	...
	homeMenuManage.initTab();// 初始化TAB点击效果
...
var homeMenuManage = {
					...
					$(this).click(
							function() {
								...
								if (tabName == "tabBar01") {// 菜单列表
									// alert("菜单列表");
									homeMenuManage.initDataList(1, 10);
								} else if (tabName == "tabBar02") {// 失效列表
									// alert("失效列表");
									homeMenuManage.initDataHisList(1, 10);
								}
	...
	showAdd : function(obj) {
		...
		$('#submitBut').show();// 按钮
		$("#submitBut").attr("onclick", "homeMenuManage.addMenu(this);");
		$('#myModalLabel').text('新增菜单');
		$('#dataDiv').show();
		homeMenuManage.initAddForm();// 初始化表单
	},
	showEdit : function(menuId) {
		tab.closeAll();// 关闭当前弹出框
		$('#submitBut').show();// 按钮
		$("#submitBut").attr("onclick", "homeMenuManage.editMenu(this);");
		$('#myModalLabel').text('编辑菜单');
		$('#dataDiv').show();
		homeMenuManage.initEditForm(menuId);// 初始化表单
	},
	showData : function(activeDiv) {
		homeMenuManage.initDataList(1, 10);// 初始化表单
	},
	...
	initDataList : function(pageNo, pageSize) {
		homeMenuManage.initClearAll();// 清除数据
	...
	initDataHisList : function(pageNo, pageSize) {
		...
		$.appAjax({
			url : WEB_ROOT + "/homeMenu/getDataHis",
			data : data,
			async : true,
			dataType : "html",
			success : function(data) {
	...
	confirmUpdateStatus : function(menuId, status) {
		...
		$("#submitBut").attr(
				"onclick",
				"homeMenuManage.updateStatusMenu('" + menuId + "','" + status
						+ "');");
		$('#myModal').modal('show');
		return rs;
	},
	updateStatusMenu : function(menuId, status) {
		...
		tab.sendAfter();// 关闭当前弹出框
		$.appAjax({
			url : WEB_ROOT + "/homeMenu/updateStatus",
			data : data,
			success : function(obj) {
				//alert(obj.success);
				if (obj.success) {
